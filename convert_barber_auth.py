...
def convert_file(html_file, jsx_file, comp_name, script_name, css_name):
    ...
    def onclick_replacer(match):
        ...
        funcs = [
            'goBack', 'addService', 'removeService', 'toggleAadhaarGuide', 
            'submitRegistration', 'handleBarberLogin', 'resetPassword'
        ]
        for func in funcs:
    ...
    # Some additional fixes for Barber Auth (checked vs defaultChecked etc.)
...
convert_file('barber-auth.html', 'BarberAuth.jsx', 'BarberAuth', 'barber-auth.js', 'auth.css')
convert_file('barber-login.html', 'BarberLogin.jsx', 'BarberLogin', 'auth.js', 'auth.css')
