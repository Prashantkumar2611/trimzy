...
const AppPage = () => {
  ...
  <div className="pincode-overlay" id="pincode-overlay" onClick={(e) => { window.handleOverlayClick(e) }}>
    <div className="pincode-modal">
      <button className="pm-close" onClick={(e) => { window.closePincodeModal() }}>✕</button>
      ...
      <div className="pm-search-container" style={{position: 'relative'}}>
        <div className="pm-input-row">
          <input className="pm-input" id="pincode-input" type="text" placeholder="e.g. Bhopal, 751001"
            onInput={(e) => { window.onPincodeInput() }} onKeyDown={(e) => { if(e.key==='Enter')lookupPincode() }} autocomplete="off" />
          <button className="pm-find-btn" id="pincode-find-btn" onClick={(e) => { window.lookupPincode() }} disabled>Find</button>
        </div>
      ...
      <div className="pm-cities">
        <button className="pm-city-chip" onClick={(e) => { window.pickCity('Bhubaneswar') }}>Bhubaneswar</button>
        <button className="pm-city-chip" onClick={(e) => { window.pickCity('Cuttack') }}>Cuttack</button>
        <button className="pm-city-chip" onClick={(e) => { window.pickCity('Puri') }}>Puri</button>
        <button className="pm-city-chip" onClick={(e) => { window.pickCity('Rourkela') }}>Rourkela</button>
        <button className="pm-city-chip" onClick={(e) => { window.pickCity('Sambalpur') }}>Sambalpur</button>
        <button className="pm-city-chip" onClick={(e) => { window.pickCity('Berhampur') }}>Berhampur</button>
      </div>
      <button className="pm-gps-btn" onClick={(e) => { window.useGPS() }}>📶 Use my current GPS location</button>
    </div>
  </div>
  ...
  <div className="loc-banner" id="loc-banner" style={{display: 'none'}}>
    <div className="loc-banner-inner">
      ...
      <button className="loc-allow-btn" onClick={(e) => { window.requestLocation() }}> Allow Location Access</button>
      <button className="loc-skip-btn" onClick={(e) => { window.dismissLocBanner() }}>Maybe later</button>
    </div>
  </div>
  ...
  <nav>
    ...
    <div className="nav-right" style={{display: 'flex', gap: '12px', alignItems: 'center', flex: '1', justifyContent: 'flex-end'}}>
      <div className="city-btn-wrap">
        <button className="city-btn" id="city-btn" onClick={(e) => { window.openPincodeModal() }}>
        ...
        <div className="nav-auth-wrapper" id="nav-auth-wrapper" style={{display: 'flex', alignItems: 'center'}}>
          <div className="profile-corner" id="profile-corner" style={{display: 'none'}}>
            <div style={{position: 'relative', width: 'fit-content'}}>
              <div className="profile-btn" id="profile-btn" onClick={(e) => { window.toggleProfileDropdown(e) }}>?</div>
              <span
            ...
            <div className="profile-dropdown" id="profile-dropdown">
              ...
              <button className="pd-item" onClick={(e) => { window.requireAuthThen('bookings');toggleProfileDropdown() }}> My
                Bookings</button>
              <button className="pd-item" onClick={(e) => { window.switchAppView('browse');toggleProfileDropdown() }}> Browse Barbers</button>
              <hr className="pd-divider" />
              <button className="pd-item pd-logout" onClick={(e) => { window.doLogout() }}> Log Out</button>
            </div>
          </div>
  ...
  <div className="login-gate" id="login-gate">
    <div className="lg-box">
      ...
      <div className="lg-btns">
        <button className="lg-btn-primary" onClick={(e) => { window.openAuthModal() }}>Create Account / Log In →</button>
      ...
      <div className="app-search-row" id="search-bar">
        <div className="app-search-wrap">
          <input className="app-search" id="search-input" type="text" placeholder="Search barber or area..."
            onInput={(e) => { window.filterBarbers() }} autocomplete="off" />
        </div>
        <select className="app-filter-select" id="sort-select" onChange={(e) => { window.filterBarbers() }}>
          <option value="rating">Top Rated</option>
          <option value="price-low">Price: Low → High</option>
          <option value="price-high">Price: High → Low</option>
          <option value="distance">Nearest First</option>
        </select>
        <select className="app-filter-select" id="area-select" onChange={(e) => { window.filterBarbers() }}>
  ...
  <div className="rating-overlay" id="rating-modal-overlay">
    <div className="rating-modal">
      ...
      <div className="rm-stars">
        <span className="rm-star" onClick={(e) => { window.setRating(1) }}>★</span>
        <span className="rm-star" onClick={(e) => { window.setRating(2) }}>★</span>
        <span className="rm-star" onClick={(e) => { window.setRating(3) }}>★</span>
        <span className="rm-star" onClick={(e) => { window.setRating(4) }}>★</span>
        <span className="rm-star" onClick={(e) => { window.setRating(5) }}>★</span>
      </div>
      <textarea className="rm-comment" id="rm-comment" placeholder="Leave a short review (optional)..." rows="3"></textarea>
      <div style={{display: 'flex', gap: '10px'}}>
        <button className="rm-submit" id="rm-submit-btn" onClick={(e) => { window.submitReview() }}>Submit Review</button>
        <button className="pm-close" onClick={(e) => { window.closeRatingModal() }}
          style={{position: 'static', width: 'auto', height: '48px', padding: '0 20px', borderRadius: '14px'}}>Cancel</button>
      </div>
    </div>
  </div>

  <div className="panel-overlay" id="panel-overlay" onClick={(e) => { window.closePanel() }}></div>
  <div className="booking-panel" id="booking-panel">
    ...
    <div className="bp-header">
      ...
      <button className="bp-close" onClick={(e) => { window.closePanel() }}>✕</button>
    </div>
    <div className="bp-steps">
      <div className="bp-step active" id="step-tab-1" onClick={(e) => { window.goToStep(1) }}>1. Service</div>
      <div className="bp-step" id="step-tab-2" onClick={(e) => { window.goToStep(2) }}>2. Date & Time</div>
      <div className="bp-step" id="step-tab-3" onClick={(e) => { window.goToStep(3) }}>3. Confirm</div>
    </div>
    <div className="bp-content-step active" id="step-1">
      <div className="service-list" id="service-list"></div>
      <div className="bp-action">
        <button className="bp-next-btn" id="next-1" onClick={(e) => { window.goToStep(2) }} disabled>Choose Date & Time →</button>
      </div>
    </div>
    <div className="bp-content-step" id="step-2">
      ...
      <div className="bp-action">
        <button className="bp-back-btn" onClick={(e) => { window.goToStep(1) }}>← Back</button>
        <button className="bp-next-btn" id="next-2" onClick={(e) => { window.goToStep(3) }} disabled>Review Booking →</button>
      </div>
    </div>
    <div className="bp-content-step" id="step-3">
      ...
      <div className="payment-options">
        {/*  Option 1: UPI  */}
        <button type="button" className="payment-option selected" id="po-upi" onClick={(e) => { window.selectPayment('upi') }}>
        ...
        {/*  Option 2: Card  */}
        <button type="button" className="payment-option" id="po-card" onClick={(e) => { window.selectPayment('card') }}>
        ...
        {/*  Option 3: Pay Barber  */}
        <button type="button" className="payment-option" id="po-cash" onClick={(e) => { window.selectPayment('cash') }}>
      ...
      <div className="bp-action">
        <button type="button" className="bp-back-btn" onClick={(e) => { window.goToStep(2) }}>← Back</button>
        <button type="button" className="bp-next-btn" id="confirm-btn" onClick={(e) => { window.handlePayment() }}>
          Proceed to Pay →
        </button>
      </div>
    </div>
  ...
  <div className="success-screen" id="success-screen">
    ...
    <div className="ss-btns">
      <button className="ss-btn-primary" onClick={(e) => { window.bookAnother() }}>Book Another</button>
      <button className="ss-btn-outline" onClick={(e) => { window.viewMyBookings() }}>My Bookings</button>
